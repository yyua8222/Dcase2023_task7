...
class DDIMSampler(object):
    ...
    def ddim_sampling(
        ...
        b = shape[0]
        # print(f"the random shape is {shape} and the x_t shape is {x_T.shape}")
        if x_T is None:
            img = torch.randn(shape, device=device)
        else:
            print("using x_T ! ")
            temp = torch.randn(shape, device=device)
            img = x_T
            # print(f"the temp noise is {temp[0]} and the  img is {img[0]}")
