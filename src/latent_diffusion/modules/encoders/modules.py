...
from latent_diffusion.util import float32_to_int16, int16_to_float32
import torchaudio
from latent_diffusion.modules.x_transformer import Encoder, TransformerWrapper
...
class CLAPAudioEmbeddingClassifierFreev2(nn.Module):
    ...
    def wav_similarity(self, waveform1, waveform2):
        ...
        with torch.no_grad():
            ...
            self.embed_mode = "audio"
            text_emb = self(waveform2)
            similarity = F.cosine_similarity(audio_emb, text_emb, dim=2)
        self.embed_mode = original_embed_mode
        return similarity.squeeze()

    def emb_similarity(self, waveform1, embd2):
        # waveform: [bs, t_steps]
        original_embed_mode = self.embed_mode
        with torch.no_grad():
            self.embed_mode = "audio"
            audio_emb = self(waveform1.cuda())
            similarity = F.cosine_similarity(audio_emb, embd2, dim=2)
        self.embed_mode = original_embed_mode
        return similarity.squeeze()
